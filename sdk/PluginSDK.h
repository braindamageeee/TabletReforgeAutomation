...
// Helper: SEH-protected read of string length
inline size_t ReadStringLenSEH(uintptr_t addr, const HostAbi* abi) {
    __try {
        return abi->memory.read_string(addr, nullptr, 0);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        OutputDebugStringA("[FetchString] SEH异常: 获取字符串长度失败\n");
        return 0;
    }
}

// Helper: SEH-protected read of string data
inline bool ReadStringDataSEH(uintptr_t addr, const HostAbi* abi, char* buffer, size_t needed) {
    __try {
        abi->memory.read_string(addr, buffer, needed);
        return true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        OutputDebugStringA("[FetchString] SEH异常: 读取字符串数据失败\n");
        return false;
    }
}

// Helper: SEH-protected read of wstring length
inline size_t ReadWStringLenSEH(uintptr_t addr, const HostAbi* abi) {
    __try {
        return abi->memory.read_wstring(addr, nullptr, 0);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        OutputDebugStringA("[FetchWString] SEH异常: 获取宽字符串长度失败\n");
        return 0;
    }
}

// Helper: SEH-protected read of wstring data
inline bool ReadWStringDataSEH(uintptr_t addr, const HostAbi* abi, wchar_t* buffer, size_t needed) {
    __try {
        abi->memory.read_wstring(addr, buffer, needed);
        return true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        OutputDebugStringA("[FetchWString] SEH异常: 读取宽字符串数据失败\n");
        return false;
    }
}
...
inline std::string FetchString(uintptr_t addr, const HostAbi* abi) {
    ...
    // First call: get the string length (SEH protected via helper)
    size_t needed = ReadStringLenSEH(addr, abi);
    
    if (needed <= 1 || needed > 4096) {
        if (needed > 4096) {
            char log[128];
            sprintf_s(log, "[FetchString] 无效字符串长度: %zu (addr=0x%p)\n", needed, (void*)addr);
            OutputDebugStringA(log);
        }
        return {};
    }
    
    // Second call: read the string data (SEH protected via helper)
    char* bufferPtr = buffer;
    std::vector<char> dynBuffer;
    if (needed > sizeof(buffer)) {
        dynBuffer.resize(needed);
        bufferPtr = dynBuffer.data();
    }
    
    if (!ReadStringDataSEH(addr, abi, bufferPtr, needed)) {
        return {};
    }
    
    // Convert to std::string (safe - no SEH here)
    std::string s(bufferPtr, needed - 1);
    
    // Validate string quality (check for mostly invalid characters)
    int validChars = 0;
    int totalChars = static_cast<int>(s.size());
    for (int i = 0; i < totalChars; ++i) {
        unsigned char c = static_cast<unsigned char>(s[i]);
        if ((c >= 32 && c <= 126) || c == ' ' || c == '\t' || c == '-' || c == '_' || c == '/' || c == '.') {
            validChars++;
        }
    }
    
    if (totalChars > 2 && validChars < totalChars / 3) {
        char log[256];
        sprintf_s(log, "[FetchString] 字符串无效: 有效字符=%d/%d, 内容='%s'\n", 
            validChars, totalChars, s.c_str());
        OutputDebugStringA(log);
        return {};
    }
...
inline std::wstring FetchWString(uintptr_t addr, const HostAbi* abi) {
    ...
    // First call: get the wstring length (SEH protected via helper)
    size_t needed = ReadWStringLenSEH(addr, abi);
    
    if (needed <= 1 || needed > 4096) return {};
    
    wchar_t* bufferPtr = buffer;
    std::vector<wchar_t> dynBuffer;
    if (needed > sizeof(buffer) / sizeof(buffer[0])) {
        dynBuffer.resize(needed);
        bufferPtr = dynBuffer.data();
    }
    
    // Second call: read the wstring data (SEH protected via helper)
    if (!ReadWStringDataSEH(addr, abi, bufferPtr, needed)) {
        return {};
    }
    
    std::wstring s(bufferPtr, needed - 1);
    return s;
}
...
struct InventoryItem {
    ...
    static InventoryItem FromAbi(const InventoryItemAbi& a, const HostAbi* abi) {
        ...
        // Read strings with logging for diagnostics
        if (a.path_addr) {
            i.Path = FetchString(a.path_addr, abi);
        }
        if (a.base_type_name_addr) {
            i.BaseTypeName = FetchString(a.base_type_name_addr, abi);
        }
        if (a.unique_name_addr) {
